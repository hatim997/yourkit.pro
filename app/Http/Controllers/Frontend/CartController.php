...
class CartController extends Controller
{
    ...
    public function getCart(string $sessionId)
    {
        ...
        if (!$carts->isEmpty()) {
            ...
            $html .= '<div class="print-charge"><h5>Total :</h5><h5><strong>' .  (HelpersHelper::formatCurrency($total)) . '</strong></h5></div>
            ...
            $html = ' <div class="container-xxl"><div class="cart-prt text-center"><span><strong> Cart is empty!</strong></span><div class="continue-prt py-4 justify-content-center">
                    <a href="' . route("frontend.home") . '" class="btn btn-outline-warning">Continue Shopping</a></div></div>';
        }
    ...
    public function edit($id)
    {
        ...
        // return $cart;
        $allColors = AttributeValue::where('attribute_id', 2)->pluck('value')->toArray();
        ...
        return view('frontend.choose-product-edit', ['bundle' => $bundle, 'meta' => $bundle->uuid, 'positions' => $positions, 'cart' => $cart, 'subcategories' => $subcategories, 'allColors' => $allColors]);
    }
